...
const zonedParts = (date: Date, timeZone: string): { day: number; minuteOfDay: number } => {
...
export const isOpenAt = (hours: OperatingHour[], now: Date, timeZone: string): boolean => {
  if (hours.length === 0) return true;
  const local = zonedParts(now, timeZone);
  const today = hours.find((hour) => hour.dayOfWeek === local.day);
  const yesterday = hours.find((hour) => hour.dayOfWeek === (local.day + 6) % 7);
...
export const contextScoreV5 = (
  ...
  if (hunger >= 4) {
    if ((meal.calories ?? 0) >= 550 || includesAny(meal.tags, ['hearty', 'filling', 'protein', 'grill'])) {
      score += 0.45;
      reasons.push('HUNGER_FIT');
    }
    if ((meal.preparationMin ?? 99) <= 20) score += 0.12;
  } else if (hunger <= 2) {
    if ((meal.calories ?? 9999) <= 500 || includesAny(meal.tags, ['light', 'salad', 'soup', 'healthy'])) {
      score += 0.4;
  ...
  if (mood) {
    const comfort = ['comfort', 'cozy', 'indulgent', 'dessert', 'pizza', 'burger'];
    const fresh = ['fresh', 'healthy', 'salad', 'bowl', 'vegetable'];
    if (['sad', 'stressed', 'tired', 'comfort'].some((key) => mood.includes(key)) && includesAny(meal.tags, comfort)) {
      score += 0.3;
      reasons.push('MOOD_FIT');
    }
    if (['fresh', 'healthy', 'energetic', 'light'].some((key) => mood.includes(key)) && includesAny(meal.tags, fresh)) {
      score += 0.3;
      reasons.push('MOOD_FIT');
    }
  }

  if (context.mealType && meal.mealType?.toLowerCase() === context.mealType.toLowerCase()) {
    score += 0.2;
  }
  return { score, reasons };
};

export const freshnessScoreV5 = (updatedAt: Date, now: Date): { score: number; stale: boolean } => {
...
export const diversifyV5 = <T extends { restaurant: { id: string }; meal: { cuisine: string | null }; score: number }>(
  ranked: T[],
  limit: number,
): T[] => {
  const selected: T[] = [];
  ...
  while (pool.length > 0 && selected.length < limit) {
    ...
    pool.forEach((item, index) => {
      ...
      const adjusted = item.score - cuisinePenalty - restaurantCount * 0.28;
    ...
    restaurantCounts.set(chosen.restaurant.id, (restaurantCounts.get(chosen.restaurant.id) ?? 0) + 1);
...
export const confidenceV5 = (scores: number[]): number => {
  ...
  const gap = Math.max(0, scores[0] - scores[1]);
  return Math.min(0.98, Math.max(0.35, 0.55 + gap / 4 + Math.min(scores[0], 6) / 30));
};
