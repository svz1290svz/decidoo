...
export const registerAdminRoutes = async (app: FastifyInstance): Promise<void> => {
  app.get('/v1/admin/dashboard', async (request, reply) => {
    try {
      ...
      const [
        ...
        prisma.recommendationLog.count({ where: { action: { not: null } } }),
      ...
      return {
        ...
        growth30d: {
          users: newUsers30d,
          restaurants: newRestaurants30d,
        },
        rates: {
          recommendationEngagement:
            recommendations === 0
              ? 0
              : Number((recommendationActions / recommendations).toFixed(4)),
          restaurantsActive:
            restaurants === 0
              ? 0
              : Number((activeRestaurants / restaurants).toFixed(4)),
        },
      };
    } catch {
  ...
  app.patch<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        const adminId = adminFromRequest(request);
        ...
        await prisma.$executeRaw`
          INSERT INTO "AuditEvent" ("id", "actorUserId", "action", "entityType", "entityId", "requestId", "metadata", "createdAt")
          VALUES (${randomUUID()}, ${adminId}, 'RESTAURANT_MODERATED', 'Restaurant', ${existing.id}, ${request.id}, ${JSON.stringify({ status: parsed.data.status, note: parsed.data.note ?? null })}::jsonb, NOW())
        `;
